...
# 历史遗留测试文件：针对恢复版前已不存在的模块（llm_client / 旧版 tool_protocol / 旧 model_factory API 等）
# 默认跳过这些文件中的测试，避免污染本地/CI 默认运行；可通过 RUN_LEGACY_TESTS=1 强制跑。
LEGACY_FILES = {
    "test_agent_e2e.py",
    "test_mcp_server.py",
    "test_model_factory.py",
    "test_new_modules.py",
    "test_new_modules_extended.py",
    "test_performance.py",
    "test_smoke.py",
...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        mod_name = _item_module_name(item)
        if mod_name not in LEGACY_FILES:
            continue
